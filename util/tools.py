...
def find_shot(agent, target, cap_=6, can_aerial=True, can_double_jump=True, can_jump=True, can_ground=True):
    ...
    min_aerial_height = 451 if agent.me.location.z < 1200 else 150
    ...
    for ball_slice in slices:
        ...
        shot = virxrlcu.parse_slice_for_shot_with_target(can_ground, can_jump, can_double_jump, can_aerial and (min_aerial_height < ball_location[2]), time_remaining, *game_info, gravity, ball_info, me, targets)
...
def find_any_shot(agent, cap_=6, can_aerial=True, can_double_jump=True, can_jump=True, can_ground=True):
    ...
    for ball_slice in slices:
        ...
        # Check if we can make a shot at this slice
        # This operation is very expensive, so we use C to improve run time
        shot = virxrlcu.parse_slice_for_shot(can_ground, can_jump, can_double_jump, can_aerial and (min_aerial_height < ball_location[2]), time_remaining, *game_info, gravity, ball_info, me)
