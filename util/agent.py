    ...
    def main(self):
        while 1:
            try:
                ...
                if not self.ready:
                    # Search for the human in the packet
                    for i in range(packet.num_cars):
                        ...
                        if not car.is_bot:
                            ...
                            self.team = car.team
                            self.name = car.name
                            print(f"Cyborg: Established connection to human in car #{self.index} on team {self.team} named {self.name}")
                            break
                ...
                try:
                    ...
                    t_file = os.path.join(self.traceback_file[0], "handle_controller"+self.traceback_file[1])
                    print(f"ERROR in {self.name}'s handle_controller; see '{t_file}'")
                    print_exc(file=open(t_file, "a"))
            except Exception:
